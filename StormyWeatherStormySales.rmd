...
```{r CV-linear-mods}
set.seed(425)
lm1_mod = train(units ~ store_nbr + item_nbr + resultspeed + heat,
                data = trn_s,
                method = "lm",
...
set.seed(425)
lm2_mod = train(units ~ store_nbr + item_nbr + resultspeed,
                data = trn_s,
                method = "lm",
                trControl = trainControl(method = "LGOCV", p = 0.8, number = 1),
                metric = "RMSE"
...
set.seed(425)
lm3_mod = train(units ~ store_nbr + item_nbr,
                data = trn_s,
                method = "lm",
                trControl = trainControl(method = "LGOCV", p = 0.8, number = 1),
                metric = "RMSE"
...
# doesnt help
set.seed(425)
lm4_mod = train(units ~ store_nbr*resultspeed + item_nbr,
                data = trn_s,
                method = "lm",
                trControl = trainControl(method = "LGOCV", p = 0.8, number = 1),
                metric = "RMSE"
...
set.seed(425)
lm5_mod = train(units ~ store_nbr*heat + item_nbr,
                data = trn_s,
                method = "lm",
                trControl = trainControl(method = "LGOCV", p = 0.8, number = 1),
                metric = "RMSE"
...
set.seed(425)
lm6_mod = train(units ~ store_nbr*tmax + item_nbr,
                data = trn_s,
                method = "lm",
                trControl = trainControl(method = "LGOCV", p = 0.8, number = 1),
                metric = "RMSE"
...
set.seed(425)
lm7_mod = train(units ~ store_nbr + tmax + item_nbr,
                data = trn_s,
                method = "lm",
                trControl = trainControl(method = "LGOCV", p = 0.8, number = 1),
                metric = "RMSE"
...
set.seed(425)
lm8_mod = train(units ~ store_nbr + item_nbr + dewpoint,
                data = trn_s,
                method = "lm",
                trControl = trainControl(method = "LGOCV", p = 0.8, number = 1),
                metric = "RMSE"
...
```{r}
trn_cut1 = trn_s
trn_cut1$concatStrItem = paste0(trn_cut1$store_nbr, sep = ", ", trn_cut1$item_nbr)
trn_cut2 = trn_s

# see which stores and items together have NEVER sold anything
noSell_df = trn_s %>%
  group_by(store_nbr, item_nbr) %>% 
  summarise("totUnits" = sum(units)) %>% 
  filter(totUnits == 0)
# 4745 out of 4995 combinations of store number and item number NEVER SELL!!!!!!!! 

# following two lines help us retrieve the indices for the combinations that never sell
noSell_strItem = data.frame("store_nbr" = noSell_df$store_nbr, "item_nbr" = noSell_df$item_nbr,
                            "concatStrItem" = paste0(noSell_df$store_nbr, sep = ", ",
                                                     noSell_df$item_nbr))
idx_noSell = which(trn_cut1$concatStrItem %in% noSell_strItem$concatStrItem)

trn_noSell = trn_s[idx_noSell,]
trn_sell = trn_s[-idx_noSell,]
```
